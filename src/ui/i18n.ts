...
const zh = {
  ...
  needsRepair: "待修复", missingNodeNote: "缺少节点笔记", missingNoteShort: "缺少笔记", missingNodeFolder: "缺少同名文件夹", nodeConflict: "同名节点已存在",
  unmanaged: "不管理", unmanagedFolder: "不管理的文件夹", exempt: "不管理", unreferenced: "未被任何笔记引用", contentsUninitialized: "Folder Nodes 尚未初始化；当前只显示结构扫描结果，自动同步尚未启用。",
  ...
  visualValueDesc: "输入 Emoji、Lucide 图标名、CSS 颜色或 Vault 图片 WikiLink；留空可删除。",
  iconGuideTitle: "如何设置节点图标",
  iconGuideIntro: "在文件列表中右键节点，选择“设置节点图标”；也可以直接编辑节点笔记的 icon 属性。",
  iconPropertyExampleLabel: "属性示例",
  iconGuideFormats: "支持 Emoji、Lucide 图标名、CSS 颜色和 Vault 图片 WikiLink；属性为列表时使用第一个有效值，留空即可删除图标。",
...
const en: typeof zh = {
  ...
  needsRepair: "Needs repair", missingNodeNote: "Missing Node Note", missingNoteShort: "Missing note", missingNodeFolder: "Missing same-named folder", nodeConflict: "Same-named node already exists",
  unmanaged: "Unmanaged", unmanagedFolder: "Unmanaged folder", exempt: "Unmanaged", unreferenced: "Not referenced by any note", contentsUninitialized: "Folder Nodes is not initialized. This is a structure preview; automatic synchronization is disabled.",
  ...
  visualValueDesc: "Enter an emoji, Lucide icon name, CSS color, or Vault image wikilink; leave empty to remove it.",
  ...
  iconGuideFormats: "Supports emoji, Lucide icon names, CSS colors, and Vault image wikilinks. A list uses its first valid value; leave the property empty to remove the icon.",
