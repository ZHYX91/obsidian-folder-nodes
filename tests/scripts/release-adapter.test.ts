...
describe("release adapter", () => {
  it("declares only the repository-specific release policy", () => {
    expect(releaseConfig).toEqual({
      ...
      publication: { repository: "ZHYX91/obsidian-folder-nodes" },
    });
  ...
  it("keeps vendored release-core bytes stable in a fresh Windows clone", () => {
    for (const file of [
      "scripts/vendor/obsidian-release-core.mjs",
      "scripts/vendor/obsidian-release-core.lock.json",
    ]) {
      const attribute = execFileSync("git", ["check-attr", "eol", "--", file], {
        cwd: projectRoot,
        encoding: "utf8",
        windowsHide: true,
      });
      expect(attribute.trim()).toBe(`${file}: eol: lf`);
    }
  });

  it("has a self-contained import closure in a standalone clone", () => {
    const isolatedRoot = mkdtempSync(path.join(tmpdir(), "folder-nodes-release-adapter-"));
    try {
