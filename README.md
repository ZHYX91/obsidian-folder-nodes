...
- Explore Folder Nodes in one Node Graph workspace view. Switch between Structure, Links, and Hybrid relations and deterministic 2D or layered 3D layouts; open the global graph, a selected subtree, or a bounded local neighborhood. Search, focus, fit, and boundary-node cues stay shared across modes.
...
- **Node Graph** provides the total switch, default relation and dimension, local depth, optional link-boundary nodes, large-graph thresholds, include-subtree rules, and exact-node or whole-subtree exclusions. Its live estimate reports the currently configured node and edge count before opening the graph.
