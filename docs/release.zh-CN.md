...
候选前运行 `npm ci`、`npm run check`、`npm run release:check` 和大目录 benchmark。隔离 Vault 主机验收必须绑定 exact candidate。Explorer、拖拽、选区右键、Visual、图墙、迁移和删除等主机能力没有日期证据时，不得在 README 或 release notes 中宣称已通过。
