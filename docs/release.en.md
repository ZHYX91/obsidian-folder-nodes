...
The Node Graph release handoff requires automated narrow-screen, coarse-pointer, and touch coverage plus current Android-emulator acceptance. The emulator covers Node Graph scopes, handles, search, 2D/3D, touch targets, and restart together with the existing no-draggable, native-move, menu, rank, and narrow-layout scenarios. Android physical devices and iOS are out of scope.
