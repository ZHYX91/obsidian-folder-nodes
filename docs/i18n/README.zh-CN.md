...
- 在同一个节点图谱工作区视图中探索 Folder Node：可切换“结构/链接/混合”关系和确定性的 2D 或分层 3D 布局，也可打开全局图谱、指定子树或有限局部邻域；搜索、聚焦、适应视图和边界节点提示在各模式间共用。
...
- **节点图谱**：总开关、默认关系与维度、局部后代深度、可选链接边界节点、大图阈值、纳入子树规则，以及精确节点或整棵子树的忽略规则；打开图谱前会实时估算当前配置下的节点数和边数。
