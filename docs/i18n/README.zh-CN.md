...
7. 桌面端可将 Folder Node 拖到另一个节点之前、之内或之后，也可把一个相册/文件条目拖入节点或面包屑；Android 使用 Obsidian 原生移动文件夹，或 Folder Nodes 的“移动”“上移”“下移”菜单动作。
...
Folder Nodes 只在本地运行，不发起网络请求。健康扫描和批量整理预览会盘点 Vault 路径；已记录的用户操作可以创建、修改、移动、重命名、合并笔记和文件夹，或将其移入回收站。批量整理始终先预览并在提交前重新核对；结构写入串行执行，路径冲突阻止提交，歧义操作失败关闭。回滚动作始终绑定原始 Vault 对象；对象已改变或同路径已被替换时拒绝恢复，不会操作新的占位对象。完整节点删除使用 Obsidian 的系统回收站路径。只有用户明确执行复制操作后，插件才会把生成的 Markdown 链接写入系统剪贴板；它从不读取剪贴板。不管理规则与其他设置保存在插件 `data.json`，结构事实保留在 Vault；节点内容、路径、视觉和诊断都不会上传。
