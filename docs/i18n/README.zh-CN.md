...
- 通过文件资源管理器浏览全局 Node Tree：使用置顶且不可折叠的根节点行；点击普通文件夹名称打开 Node Note，其展开箭头仍只负责展开；隐藏重复 canonical note。桌面端支持 before/into/after 拖放；Android 使用 Obsidian 原生移动文件夹和 Folder Nodes 的“移动”“上移”“下移”。
...
- 在同一个节点图谱工作区视图中探索 Folder Node：可切换“结构/链接/混合”关系、默认从左到右的易读 2D 思维导图布局（也可改回从上到下）和分层 3D，也可打开全局图谱、指定子树或有限局部邻域；搜索、聚焦、适应视图和边界节点提示在各模式间共用。
...
- 无需初始化即可立即识别完整与不完整节点；可选批量整理使用响应式精确路径预览，冲突阻止提交，健康检查严格只读。
...
- 支持桌面版与 Android 版 Obsidian。当前候选的 Android 模拟器验收与桌面验收分别记录；真机行为仍是独立声明。
...
7. 从节点内容、命令面板或 Folder Node/Node Note 右键菜单打开“节点图谱”。“打开此节点的子图谱”从指定节点向下显示；“打开此节点的局部图谱”显示其父节点、设置深度内的后代和直接链接邻居。
...
- **常规**：界面语言、“不管理的 Markdown 文件/不管理的文件夹”两个统一规则组、可选的预览优先批量整理和只读健康检查。
...
- **节点图谱**：总开关、默认关系与维度、2D 布局方向、局部后代深度、可选链接边界节点、大图阈值、纳入子树规则，以及精确节点或整棵子树的忽略规则。2D 默认从左到右：横向卡片构成稳定层级，已解析的笔记链接仍可跨层、跨分支连接；打开图谱前会实时估算当前配置下的节点数和边数。
...
- 仅链接模式仍沿用确定性的 Folder Node 结构坐标系，不使用随机力导向布局。超大图谱会切换到 Canvas，并限制密集总览中的边；当全 Vault 总览过密时，应使用子树/局部范围或纳入/忽略规则。

## 隐私与安全

Folder Nodes 只在本地运行，不发起网络请求。健康扫描和批量整理预览会盘点 Vault 路径；节点图谱读取本地 Folder Node 结构和 Obsidian Metadata Cache，不上传笔记内容，也不建立第二套全 Vault 链接扫描。已记录的用户操作可以创建、修改、移动、重命名、合并笔记和文件夹，或将其移入回收站。批量整理始终先预览并在提交前重新核对；结构写入串行执行，路径冲突阻止提交，歧义操作失败关闭。回滚动作始终绑定原始 Vault 对象；对象已改变或同路径已被替换时拒绝恢复，不会操作新的占位对象。完整节点删除使用 Obsidian 的系统回收站路径。只有用户明确执行复制操作后，插件才会把生成的 Markdown 链接写入系统剪贴板；它从不读取剪贴板。不管理规则、节点图谱范围规则与其他设置保存在插件 `data.json`，结构事实保留在 Vault；节点内容、路径、视觉和诊断都不会上传。
