...
- 在同一个渐进展开的节点图谱工作区视图中探索 Folder Node。结构始终是层级骨架；“显示链接”是默认关闭的独立开关，只叠加已解析 canonical-note 链接而不移动节点。图谱支持易读的从左到右 2D（也可从上到下）、分层 3D、全局/子树/局部范围、逐分支展开把手、范围展开、原生搜索、聚焦与适应视图；收起包含当前焦点的分支时，焦点自动提升到最近可见祖先。
