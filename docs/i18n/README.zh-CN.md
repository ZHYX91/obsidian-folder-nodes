...
- 通过 Node Note 中可移植的 `folderNodeHidden: true` 属性隐藏完整受管理节点及其后代。隐藏只影响文件列表、节点内容和 Folder Nodes 节点图谱；Obsidian 搜索、快速切换、反向链接、原生图谱、链接与直接打开均不受影响。“常规”可以在不删除标记的情况下忽略全部隐藏标记；Ribbon 或命令面板可以仅在当前会话临时显示它们。
...
- **常规**：界面语言、是否应用隐藏标记、“不管理的 Markdown 文件/不管理的文件夹”两个统一规则组、可选的预览优先批量整理和只读健康检查。
...
- **节点图谱**：总开关、默认维度、2D 布局方向、大图阈值、纳入子树规则，以及精确节点或整棵子树的忽略规则。结构始终显示，新建图谱的“显示链接”默认关闭。2D 默认从左到右：横向卡片构成稳定层级，开启链接叠加后已解析链接仍可跨层、跨分支连接；打开图谱前会实时估算当前配置的节点数。
...
Folder Nodes 只在本地运行，不发起网络请求。健康扫描和批量整理预览会盘点 Vault 路径；节点图谱读取本地 Folder Node 结构、Metadata Cache 和共用反向引用索引，不上传笔记内容，也不建立另一套全 Vault 链接扫描。已记录的用户操作可以创建、修改、移动、重命名、合并笔记和文件夹，或将其移入回收站。批量整理始终先预览并在提交前重新核对；结构写入串行执行，路径冲突阻止提交，歧义操作失败关闭。回滚动作始终绑定原始 Vault 对象；对象已改变或同路径已被替换时拒绝恢复，不会操作新的占位对象。完整节点删除使用 Obsidian 的系统回收站路径。只有用户明确执行复制操作后，插件才会把生成的 Markdown 链接写入系统剪贴板；它从不读取剪贴板。不管理规则、节点图谱过滤规则与其他设置保存在插件 `data.json`；图谱工作区状态只持久化范围、焦点、维度与是否显示链接。分支展开和搜索快照只在当前会话存在，重启后恢复安全的一层默认。结构事实保留在 Vault；节点内容、路径、视觉和诊断都不会上传。
